# code base from https://github.com/zafercavdar/fasttext-langdetect
...
def rank(text: str, nb_pred: int = 5, low_memory=False) -> List[Dict[str, Union[str, float]]]:
    model = get_or_load_model(low_memory)
    labels, scores = model.predict(text, k=nb_pred)
    return [{"lang": labels[i].replace("__label__", ''), "score": scores[i]} for i in range(len(labels))]